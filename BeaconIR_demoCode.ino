...
void stateMachine()
{
    switch(state)
    {
        ...
        iRlen = IR.IsDta();
        while(iRlen == 0)         // wait for ir data, if more than 10 seconds, as time out
        {
            iRlen = IR.IsDta();
            delay(10);
            tout++;
...
//        iRlen = IR.IsDta();             // get ir data length

#if DEBUG_MODE
        Serial.print("iRlen = ");Serial.println(iRlen);
#endif
        IR.Recv(pIrRev);                 // get ir data if buff: pIrRev
        ...
        IR.Send(&blueDataRevBuf[4], 38);
#if DEBUG_MODE
        Serial.println("xxxxxxxxxxxxxxxxxxxx:");
        for(int i = 0; i<blueDataRevBuf[4]+1; i++)
        {
            Serial.print(blueDataRevBuf[4+i]);
            Serial.print("\t");
        }
        Serial.println();
#endif
